...
In order to make a quantitative analysis of the results obtained in the closure
tests, we discuss several estimators, which are computed from the outcome of the
closure test fits. These are stochastic variables and therefore we need to
understand their probability distributions in order to validate our fitting
procedure. We begin this section by defining estimators in data space, \ie\
estimators that are computed from the model predictions for a set of
experimental data points. Having defined the estimators, we define criteria to
characterise faithful uncertainties. We conclude this section with a discussion
of the predictions that can be obtained for our estimators in the case of a
linear model, where analytical calculations can be performed. While these cannot
be applied directly to the NNPDF fits, they are useful examples that illustrate
the expected behaviour of these quantities.  
