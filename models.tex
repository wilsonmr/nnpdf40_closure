...
because in determining $\testset{\obspostcov}$ we set $\testset{\obspriorcov}^{-1}$
but in Eq.~\ref{eq:BiasTraceObsPost} we require $\testset{\obspriorcov}^{-1}$. However
we understand Eq.~\ref{eq:BiasTraceObsPost} as a comparison between
